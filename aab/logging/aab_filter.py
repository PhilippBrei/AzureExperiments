...
class AABFachlicherFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        if not isinstance(integrationsfluss, IntegrationsFluss):
            raise ValueError("Arg[0] muss vom typ " + str(type(IntegrationsFluss)
                                                          ) + " sein, ist aber" + str(type(integrationsfluss)))

        if not isinstance(integrationslauf, IntegrationsLauf):
            raise ValueError(
                "Arg[1] muss vom typ date sein, ist aber" + str(type(integrationslauf)))
